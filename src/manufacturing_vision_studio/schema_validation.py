...
def load_schema(name: str, version: str = "1.0.0") -> dict[str, Any]:
    ...
    if version == "1.1.0":
        directory = directory.parent / "v1.1"
    elif version != "1.0.0":
        raise EvidenceError("Unsupported schema version", code="UNSUPPORTED_SCHEMA_VERSION")
...
def validate_document(document: dict[str, Any], schema_name: str) -> None:
    ...
    schema = load_schema(schema_name, document["schema_version"])
...
def schema_id(schema_name: str, version: str = "1.0.0") -> str:
    identifier = load_schema(schema_name, version).get("$id")
